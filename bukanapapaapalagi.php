...
<body>
    <?php
    // Memulai atau memulihkan session
    session_start();
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (count($selectedMatkuls) == 49) {
            ...
            // Menyimpan pilihan mata kuliah ke dalam session
            $_SESSION['selectedMatkuls'] = $selectedMatkuls;
        ...
        // Memulihkan pilihan mata kuliah dari session (jika ada)
        if (isset($_SESSION['selectedMatkuls'])) {
            $selectedMatkuls = $_SESSION['selectedMatkuls'];
        } else {
            $selectedMatkuls = array();
        }
    }
    ?>
    ...
    <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        ...
        for ($i = 1; $i <= 49; $i++) {
            ...
            foreach ($prasyarat as $matkul => $prereq) {
                $selected = in_array($matkul, $selectedMatkuls) ? "selected" : "";
